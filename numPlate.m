...
if isequal(filename, 0)
    ...
    colorImage = imread(fullfile(filepath, filename));
end
...
[mserRegions, mserConnComp] = detectMSERFeatures(I, ... 
    "RegionAreaRange",[400 2000],"ThresholdDelta",3);
...
filterIdx = filterIdx | [mserStats.Extent] < 0.5 | [mserStats.Extent] > 0.9;
...
% Crop the detected text regions
croppedTextImage = imcrop(colorImage, textBBoxes(1,:)); % Crop the first detected text region

% Display the cropped text region
figure;
imshow(croppedTextImage);
title("Cropped Text Region");

% Use OCR on the cropped text region
